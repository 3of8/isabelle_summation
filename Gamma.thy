...
lemma tendsto_complex_of_real_iff: "((\<lambda>x. complex_of_real (f x)) ---> of_real c) F = (f ---> c) F"
...
lemma rGamma_series_nonpos_Ints_LIMSEQ: "z \<in> \<int>\<^sub>\<le>\<^sub>0 \<Longrightarrow> rGamma_series z ----> 0"
  by (elim nonpos_Ints_cases', hypsubst, subst tendsto_cong, rule rGamma_series_minus_of_nat, simp)

lemma Gamma_series_nonpos_Ints_LIMSEQ: "z \<in> \<int>\<^sub>\<le>\<^sub>0 \<Longrightarrow> Gamma_series z ----> 0"
  by (elim nonpos_Ints_cases', hypsubst, subst tendsto_cong, rule Gamma_series_minus_of_nat, simp)

lemma Gamma_series'_nonpos_Ints_LIMSEQ: "z \<in> \<int>\<^sub>\<le>\<^sub>0 \<Longrightarrow> Gamma_series' z ----> 0"
...
lemma Gamma_series_Gamma_series': 
  ...
  shows   "(\<lambda>n. Gamma_series' z n / Gamma_series z n) ----> 1"
proof (rule Lim_transform_eventually)
  ...
  qed
  have "(\<lambda>x. z / of_nat x) ----> 0"
    by (rule tendsto_norm_zero_cancel)
       (insert tendsto_mult[OF tendsto_const[of "norm z"] lim_inverse_n], 
        simp add:  norm_divide inverse_eq_divide)
  from tendsto_add[OF this tendsto_const[of 1]] show "(\<lambda>n. z / of_nat n + 1) ----> 1" by simp
qed
...
lemma ln_Gamma_complex_LIMSEQ: "(z :: complex) \<notin> \<int>\<^sub>\<le>\<^sub>0 \<Longrightarrow> ln_Gamma_series z ----> ln_Gamma z"
...
proof (rule Lim_transform)
  show "(\<lambda>n. ln_Gamma_series z n + of_real (harm n - ln (of_nat n)) * z + ln z) ----> ?s"
    (is "?g ----> _") 
  ...
  qed
  show "(\<lambda>n. (\<Sum>k<n. ?f k) - ?g n) ----> 0" by (subst tendsto_cong[OF A]) simp_all
qed
...
proof -
  ...
  qed
  finally have "(\<lambda>a. setsum f {..<a}) ----> lim (\<lambda>m. setsum f {..<m + k})"
...
class Gamma = real_normed_field + complete_space +
  ...
  assumes differentiable_rGamma_aux1: 
    "(\<And>n. z \<noteq> - of_nat n) \<Longrightarrow>
     let d = (THE d. filterlim (\<lambda>n. \<Sum>k<n. inverse (of_nat (Suc k)) - inverse (z + of_nat k)) 
               (nhds d) sequentially) - scaleR euler_mascheroni 1
  ...
  assumes rGamma_series_aux: "(\<And>n. z \<noteq> - of_nat n) \<Longrightarrow> 
             let fact' = (\<lambda>n. setprod of_nat {1..n});
                 exp = (\<lambda>x. THE e. filterlim (\<lambda>n. \<Sum>k<n. x^k /\<^sub>R fact k) (nhds e) sequentially);
...
proof (cases "z \<in> \<int>\<^sub>\<le>\<^sub>0")
  case False
  hence "(\<lambda>n. inverse (rGamma_series z n)) ----> inverse (rGamma z)"
...
proof -
  ...
  qed
  moreover have "(\<lambda>n. ?f n * rGamma_series z n) ----> ((z+1) * 0 + 1) * rGamma z"
    by (intro tendsto_intros lim_inverse_n)
  hence "(\<lambda>n. ?f n * rGamma_series z n) ----> rGamma z" by simp
  ultimately have "(\<lambda>n. z * rGamma_series (z + 1) n) ----> rGamma z"
    by (rule Lim_transform_eventually)
  moreover have "(\<lambda>n. z * rGamma_series (z + 1) n) ----> z * rGamma (z + 1)"
...
lemma Gamma_seriesI:
  assumes "(\<lambda>n. g n / Gamma_series z n) ----> 1"
  shows   "g ----> Gamma z"
proof (rule Lim_transform_eventually)
  ...
  from assms have "(\<lambda>n. g n / Gamma_series z n * Gamma_series z n) ----> 1 * Gamma z"
    by (intro tendsto_intros)
  thus "(\<lambda>n. g n / Gamma_series z n * Gamma_series z n) ----> Gamma z" by simp
qed

lemma Gamma_seriesI':
  assumes "f ----> rGamma z"
  assumes "(\<lambda>n. g n * f n) ----> 1"
...
proof (rule Lim_transform_eventually)
  ...
  from tendstoD[OF assms(2), OF this] show "eventually (\<lambda>n. g n * f n / f n = g n) sequentially"
    by (force elim!: eventually_mono simp: dist_real_def dist_0_norm)
  from assms have "(\<lambda>n. g n * f n / f n) ----> 1 / rGamma z"
    by (intro tendsto_divide assms) (simp_all add: rGamma_eq_zero_iff)
  thus "(\<lambda>n. g n * f n / f n) ----> Gamma z" by (simp add: Gamma_def divide_inverse)
qed
...
proof -
  ...
  proof (cases "z \<in> \<int>\<^sub>\<le>\<^sub>0")
    ...
    proof (rule Lim_transform_eventually)
      ...
      thus "(\<lambda>n. exp (-ln_Gamma_series z n)) ----> exp (- ln_Gamma z)"
  ...
  next
    ...
    also have "rGamma_series \<dots> ----> 0"
...
next
  ...
  from has_field_derivative_rGamma_complex_no_nonpos_Int[OF this]
    show "let d = (THE d. (\<lambda>n. \<Sum>k<n. inverse (of_nat (Suc k)) - inverse (z + of_nat k))
                       ----> d) - euler_mascheroni *\<^sub>R 1 in  (\<lambda>y. (rGamma y - rGamma z + 
              rGamma z * d * (y - z)) /\<^sub>R  cmod (y - z)) -- z --> 0"
...
next
  ...
  thus "let fact' = \<lambda>n. setprod of_nat {1..n}; 
            exp = \<lambda>x. THE e. (\<lambda>n. \<Sum>k<n. x ^ k /\<^sub>R fact k) ----> e;
            pochhammer' = \<lambda>a n. \<Prod>n = 0..n. a + of_nat n
        in  (\<lambda>n. pochhammer' z n / (fact' n * exp (z * ln (real_of_nat n) *\<^sub>R 1))) ----> rGamma z"
...
next
  ...
  thus "let d = (THE d. (\<lambda>n. \<Sum>k<n. inverse (of_nat (Suc k)) - inverse (x + of_nat k))
                       ----> d) - euler_mascheroni *\<^sub>R 1 in  (\<lambda>y. (rGamma y - rGamma x + 
              rGamma x * d * (y - x)) /\<^sub>R  norm (y - x)) -- x --> 0"
      by (simp add: has_field_derivative_def has_derivative_def Digamma_def sums_def [abs_def]
                    netlimit_at of_real_def[symmetric] suminf_def)
next
  fix n :: nat
  ...
  thus "let x = - of_nat n in (\<lambda>y. (rGamma y - rGamma x - (- 1) ^ n * setprod of_nat {1..n} *
                  (y - x)) /\<^sub>R norm (y - x)) -- (x::real) --> 0"
    by (simp add: has_field_derivative_def has_derivative_def fact_altdef netlimit_at Let_def)
next
  fix x :: real
  have "rGamma_series x ----> rGamma x"
  proof (rule Lim_transform_eventually)
    show "(\<lambda>n. Re (rGamma_series (of_real x) n)) ----> rGamma x" unfolding rGamma_real_def
      by (intro tendsto_intros)
  qed (insert rGamma_series_real, simp add: eq_commute)
  thus "let fact' = \<lambda>n. setprod of_nat {1..n}; 
            exp = \<lambda>x. THE e. (\<lambda>n. \<Sum>k<n. x ^ k /\<^sub>R fact k) ----> e;
            pochhammer' = \<lambda>a n. \<Prod>n = 0..n. a + of_nat n
        in  (\<lambda>n. pochhammer' x n / (fact' n * exp (x * ln (real_of_nat n) *\<^sub>R 1))) ----> rGamma x"
    by (simp add: fact_altdef pochhammer_Suc_setprod rGamma_series_def [abs_def] exp_def
                  of_real_def [symmetric] suminf_def sums_def [abs_def])
qed
...
proof -
  have "(\<lambda>n. ln_Gamma_series (complex_of_real x) n) ----> ln_Gamma (of_real x)" using assms
  ...
  ultimately have "(\<lambda>n. complex_of_real (ln_Gamma_series x n)) ----> ln_Gamma (of_real x)"
    by (subst tendsto_cong) assumption+
  from tendsto_Re[OF this] show ?thesis by (auto simp: convergent_def)
qed

lemma ln_Gamma_real_LIMSEQ: "(x::real) > 0 \<Longrightarrow> ln_Gamma_series x ----> ln_Gamma x"
...
lemma rGamma_zeros: 
  "(\<lambda>z. rGamma z / (z + of_nat n)) -- (- of_nat n) --> ((-1)^n * fact n :: 'a :: Gamma)"
...
lemma Gamma_residues: 
  "(\<lambda>z. Gamma z * (z + of_nat n)) -- (- of_nat n) --> ((-1)^n / fact n :: 'a :: Gamma)"
proof (subst tendsto_cong)
  let ?c = "(- 1) ^ n / fact n :: 'a"
  from eventually_at_ball'[OF zero_less_one, of "- of_nat n :: 'a" UNIV]
  ...
  have "(\<lambda>z. inverse (rGamma z / (z + of_nat n))) -- - of_nat n --> 
  ...
  finally show "(\<lambda>z. inverse (rGamma z / (z + of_nat n))) -- - of_nat n --> ?c" .
qed
...
lemma Gamma_series_euler':
  ...
  shows "(\<lambda>n. Gamma_series_euler' z n) ----> Gamma z"
...
proof (cases "z \<in> \<int>\<^sub>\<le>\<^sub>0")
  ...
  also from True have "Gamma_series_weierstrass \<dots> ----> Gamma z"
...
next
  ...
  have "(\<lambda>n. \<Sum>k=1..n. z / of_nat k - ln (1 + z / of_nat k)) ----> ln_Gamma z + euler_mascheroni * z + ln z"
...
proof (cases "z \<in> \<int>\<^sub>\<le>\<^sub>0")
  case True
  then obtain n where "z = - of_nat n" by (elim nonpos_Ints_cases')
  also from True have "rGamma_series_weierstrass \<dots> ----> rGamma z"
...
next
  ...
  also from False have "\<dots> ----> inverse (Gamma z)"
...
lemma sin_product_formula_complex:
  fixes z :: complex
  shows "(\<lambda>n. of_real pi * z * (\<Prod>k=1..n. 1 - z^2 / of_nat k^2)) ----> sin (of_real pi * z)"
proof -
...
lemma sin_product_formula_real:
  "(\<lambda>n. pi * (x::real) * (\<Prod>k=1..n. 1 - x^2 / of_nat k^2)) ----> sin (pi * x)"
proof -
...
lemma sin_product_formula_real':
  ...
  shows   "(\<lambda>n. (\<Prod>k=1..n. 1 - x^2 / of_nat k^2)) ----> sin (pi * x) / (pi * x)" 
  using tendsto_divide[OF sin_product_formula_real[of x] tendsto_const[of "pi * x"]] assms
  by simp

theorem basel_problem: "(\<lambda>n. 1 / (n + 1)\<^sup>2) sums (pi\<^sup>2 / 6)"
proof -
...
lemma Gamma_binomial:
  "(\<lambda>n. ((z + of_nat n) gchoose n) * exp (-z * of_real (ln (of_nat n)))) ----> rGamma (z+1)"
proof (cases "z = 0")
  case False
  show ?thesis
  proof (rule Lim_transform_eventually)
    ...
    from False have "(\<lambda>n. rGamma_series z n / z) ----> rGamma z / z" by (intro tendsto_intros)
    also from False have "rGamma z / z = rGamma (z + 1)" using rGamma_plus1[of z] 
      by (simp add: field_simps)
    finally show "(\<lambda>n. rGamma_series z n / z) ----> rGamma (z+1)" .
  qed
qed (simp_all add: binomial_gbinomial [symmetric])

lemma fact_binomial_limit: 
  "(\<lambda>n. of_nat ((k + n) choose n) / of_nat (n ^ k) :: 'a :: Gamma) ----> 1 / fact k"
proof (rule Lim_transform_eventually)
...
lemma binomial_asymptotic: 
  "(\<lambda>n. of_nat ((k + n) choose n) / (of_nat (n ^ k) / fact k) :: 'a :: Gamma) ----> 1"
